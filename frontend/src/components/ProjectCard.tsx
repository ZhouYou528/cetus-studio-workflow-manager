...
export default function ProjectCard({ project, tasks, completions, updateCompletions, expanded, onToggle, onEdit, onDelete, isPast, onSplit }: Props) {
  ...
  let urgencyLabel = '', urgencyColor = 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400';
  if (daysUntil < 0) { urgencyLabel = t('finished'); urgencyColor = 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'; }
  else if (daysUntil === 0) { urgencyLabel = t('shoot_day_today'); urgencyColor = 'bg-rose-100 text-rose-700'; }
  else if (daysUntil === 1) { urgencyLabel = t('shoot_day_tomorrow'); urgencyColor = 'bg-amber-100 text-amber-700'; }
  else if (daysUntil <= 3) { urgencyLabel = t('days_until_shoot', { n: daysUntil }); urgencyColor = 'bg-amber-100 text-amber-700'; }
  else if (daysUntil <= 7) { urgencyLabel = t('days_until_shoot', { n: daysUntil }); urgencyColor = 'bg-blue-100 text-blue-700'; }
  else { urgencyLabel = t('days_until_shoot', { n: daysUntil }); }

  return (
    <div className={`bg-white dark:bg-slate-900 rounded-xl border overflow-hidden transition ${isPast ? 'border-slate-200 dark:border-slate-700 opacity-75' : 'border-slate-200 dark:border-slate-700 hover:shadow-md'}`}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <h3 className="font-semibold text-slate-900 dark:text-slate-100">{project.clientName}</h3>
              <span className="text-xs px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded">{project.shootType}</span>
      ...
      {expanded && (
        <div className="border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-4">
          <div className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">{t('project_card_timeline')}</div>
          {project.notes && <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-lg p-2.5 mb-3 text-sm text-amber-900 dark:text-amber-100">💡 {project.notes}</div>}
          <div className="space-y-2">
            {tasks.map(x => {
              ...
              let timingLabel = '';
              if ((x.daysBeforeShoot ?? 0) > 0) timingLabel = t('days_before_shoot', { n: x.daysBeforeShoot ?? 0 });
              else if (x.daysBeforeShoot === 0) timingLabel = t('on_shoot_day');
              else timingLabel = t('days_after_shoot', { n: Math.abs(x.daysBeforeShoot ?? 0) });
              return (
                <div key={x.id} className={`bg-white dark:bg-slate-900 rounded-lg p-2.5 flex items-center gap-2.5 ${isOverdue ? 'border border-rose-200' : isToday ? 'border border-blue-200' : ''}`}>
                  <button onClick={(e) => { e.stopPropagation(); updateCompletions({ ...completions, [x.completionKey]: !isCompleted }); }}
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${isCompleted ? 'bg-emerald-500 border-emerald-500 task-done' : 'border-slate-300 dark:border-slate-600'}`}>
                  ...
                  {import.meta.env.DEV && onSplit && (
                    <button onClick={(e) => { e.stopPropagation(); onSplit({ ...x, name: `${x.name} - ${project.clientName}${project.shootType}` }); }}
