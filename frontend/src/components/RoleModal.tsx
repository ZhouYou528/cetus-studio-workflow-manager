    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-md p-5 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{role ? t('modal_edit_role') : t('modal_add_role')}</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('role_name')}</label>
          ...
          <div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('role_icon')}</label>
            <input value={icon} onChange={e => setIcon(e.target.value)}
              className="w-full mt-1 px-3 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-900" maxLength={2} />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('role_color')}</label>
          ...
          <div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('role_basic_duties')}</label>
            <textarea value={duties} onChange={e => setDuties(e.target.value)} rows={3}
              className="w-full mt-1 px-3 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 resize-none" />
          </div>
          <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
            <input type="checkbox" checked={isAssistant} onChange={e => setIsAssistant(e.target.checked)} className="rounded" />
            <span>{t('role_is_assistant')}</span>
          </label>
        </div>
        <div className="flex gap-2 mt-5">
          ...
          <button onClick={() => name.trim() && onSave({ name, icon, duties, isAssistant, color })}
            disabled={!name.trim()} className="flex-1 px-4 py-2 bg-slate-900 text-white rounded-lg text-sm hover:bg-slate-800 disabled:opacity-50">{t('save')}</button>
        </div>
      </div>
    </div>
  );
