...
type Props = {
  user: User;
  roles: Role[];
  // selfEdit=true 时:只显示 name 字段(用户改自己的显示名),role/assignedRoles 不可改
  selfEdit?: boolean;
  onClose: () => void;
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md p-5 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold">{selfEdit ? '编辑我的资料' : '编辑用户权限'}</h3>
          <button onClick={onClose} className="p-1 hover:bg-slate-100 rounded"><X className="w-5 h-5" /></button>
        </div>

        <div className="space-y-4">
          <div>
            <div className="text-sm font-medium text-slate-700">邮箱(不可改)</div>
            <div className="mt-1 px-3 py-2 bg-slate-100 rounded-lg text-sm font-mono text-slate-800 break-all">{user.email}</div>
          </div>

          <div>
            <label className="text-sm font-medium text-slate-700">显示名 <span className="text-xs text-slate-400 font-normal">(可选,推荐用真实姓名方便协作)</span></label>
            <input
              value={name}
              onChange={e => setName(e.target.value)}
              maxLength={40}
              placeholder={selfEdit ? '比如「张小璐」' : '比如「张三」'}
